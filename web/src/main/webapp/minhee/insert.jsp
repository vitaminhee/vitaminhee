		...
		<div class="container mt-3">
			<div class="row">
				<div class="col-md-12 d-flex justify-content-between align-items-center">
					<div>
						<div id="totalCost">선택상품 금액 : 60,000원</div>
...
function changeQuantity(button, delta) {
	// button: 버튼 요소, delta: 수량을 변경할 값(양수나 음수)
	
	// button의 부모 요소의 부모 요소에서 '.quantity-input' 클래스를 가진 input 요소를 찾음
	const input = button.parentElement.parentElement.querySelector('.quantity-input');
	
	// input 요소의 현재 값을 정수로 변환하여 currentValue에 저장
	let currentValue = parseInt(input.value);
	
	// currentValue가 숫자가 아니면 1로 초기화
	if (isNaN(currentValue)) {
		currentValue = 1;
	}
	
	// currentValue에 delta를 더하여 새로운 값을 newValue에 저장
	let newValue = currentValue + delta;
	
	// newValue가 1보다 작으면 1로 설정 (최소값 제한)
	if (newValue < 1) {
		newValue = 1;
	// newValue가 100보다 크면 100으로 설정 (최대값 제한)
	} else if (newValue > 100) {
		newValue = 100;
	}
	
	// input 요소의 값을 newValue로 설정
