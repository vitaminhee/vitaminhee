...
public class MemberDAO extends DAO{
	...
	public LoginVO login(LoginVO loginvo) throws Exception{ //loginvo가 넘어옴
		// 결과를 저장할 수 있는 변수 선언.
		LoginVO vo = null;
		try {
			// 1. 드라이버 확인 - DB
	...
	final String LOGIN = "select m.id, m.name, m.gradeNo, "
			...
			+ " from member m, grade g " // 조인해라잉
			+ " where (id = ? and pw = ? and status = '정상') " // 탈퇴했는데 로그인되면 당근 안됨
			+ " and (g.gradeNo = m.gradeNo) "; // 두 번째 조인
